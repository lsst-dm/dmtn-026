...
Between the namespace declaration (``using namespace lsst::afw::math;``) and start of the
``PYBIND11_PLUGIN`` macro we can define a template function to declare the ``minimize`` function:
...
This is an overloaded function, so we define it in the usual way but add ``"paremeter"_``
